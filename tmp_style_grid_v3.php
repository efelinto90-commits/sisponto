...
// 1. Find markers using partial strings to avoid encoding issues
$startPat = 'o 4: Quadro de Hor';
$endPat = 'o 5: Dados Banc';

$startPos = strpos($c, $startPat);
$endPos = strpos($c, $endPat);

if ($startPos !== false && $endPos !== false) {
    // Correct startPos to the beginning of the comment <!--
    $actualStart = strrpos(substr($c, 0, $startPos), '<!--');
    // Correct endPos to the beginning of the comment <!--
    $actualEnd = strrpos(substr($c, 0, $endPos), '<!--');

    if ($actualStart !== false && $actualEnd !== false) {
        $oldBlock = substr($c, $actualStart, $actualEnd - $actualStart);
        
        $newGridHtml = '<!-- SeÃ§Ã£o 4: Quadro de HorÃ¡rios Semanal (Estilo E-Social) -->
        ...
        $c = substr_replace($c, $newGridHtml . "\n\n                        ", $actualStart, $actualEnd - $actualStart);
        
        // 2. Ensure replicarSegundaRelogio is present
    ...
    window.replicarSegundaRelogio = function() {
        ...
        echo "E-Social styling successfully applied using partial matching.\n";
    } else {
        echo "Could not find comment start tags.\n";
    }
} else {
    echo "Partial markers NOT found. startPat: " . ($startPos !== false ? "OK" : "MISSING") . ", endPat: " . ($endPos !== false ? "OK" : "MISSING") . "\n";
}
?>
